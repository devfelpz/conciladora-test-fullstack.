...
public class CSVService : ICSVService
{
    ...
    public CSVService(AppDbContext db, PlacaService placa) { _db = db; _placa = placa; }

    public async Task<CSVResponseDto> Import(IFormFile csv)
    {
        ...
        while (!r.EndOfStream)
        {
            ...
            try
            {
                ...
                if (cols.Length > 7)
                {
                    var mensalistaStr = NormalizeString(cols[7]);
                    mensalista = mensalistaStr switch
                    {
                        "true" or "sim" => true,
                        "false" or "não" => false,
                        _ => false
        ...
        var resultado = new CSVResponseDto(processados, inseridos, erros);
        return resultado;
    }



    private string NormalizeString(string s) => string.IsNullOrWhiteSpace(s) ? "" :
